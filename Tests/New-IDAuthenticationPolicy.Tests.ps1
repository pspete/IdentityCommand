Describe $($PSCommandPath -Replace '.Tests.ps1') {
    ...
    InModuleScope $(Split-Path (Split-Path (Split-Path -Parent $PSCommandPath) -Parent) -Leaf ) {
        ...
        BeforeEach {
            ...
            $response = New-IDAuthenticationPolicy -PolicyName 'TestPolicy'
        ...
        Context 'Input' {
            ...
            It 'sends request' {
                ...
                # One call to fetch existing policy links (Get-IDAuthenticationPolicyLink) plus one to save the new policy block
