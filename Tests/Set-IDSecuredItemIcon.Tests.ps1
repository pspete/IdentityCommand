Describe $($PSCommandPath -Replace '.Tests.ps1') {
    ...
    InModuleScope $(Split-Path (Split-Path (Split-Path -Parent $PSCommandPath) -Parent) -Leaf ) {
        ...
        BeforeEach {
            ...
            $TestFile = Join-Path $TestDrive 'icon.png'
            Set-Content -Path $TestFile -Value 'fake image bytes' -NoNewline

            $response = Set-IDSecuredItemIcon -ItemKey 'someitemkey' -Path $TestFile
        ...
        Context 'Input' {
            ...
            It 'sends request to expected endpoint' {
                ...
                Assert-MockCalled Invoke-IDRestMethod -ParameterFilter {
                    ...
                    $URI -eq 'https://somedomain.id.cyberark.cloud/UPRest/UploadSecuredItemIcon'
